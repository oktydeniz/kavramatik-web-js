...
@section('content')
    ...
    <div class="container mt-2">
        <a href="{{route('eslestirme.karsilastirmalar')}}" class="btn btn-sm btn-primary"> <i class="fa fa-plus"></i>
            Eşleştirme</a>
                ...
                <div class="col-md-4 col-sm-6 m-10">
                    <div class="card card-block bg_bg mt-3 " style="width: 300px;height:400px;">
                        <img id="<?php echo $col1;  ?>" class="color_clicked imageOne"
                            src="data:image/jpeg;base64,{{ base64_encode($item->opposite_one_image) }}"
                            alt="{{ $item->opposite_one_image_text }}">
                        <h5 class="card-title mt-3 mb-3 category_text imageOneText">{{ $item->opposite_one_image_text }}
                        </h5>
                    </div>
                </div>
                <script>
...
btnSpeak.addEventListener('click', ()=> {
    var toSpeak = new SpeechSynthesisUtterance("{{ $item->opposite_one_image_text }}");
                ...
                <div class="col-md-4 col-sm-6 m-10">
                    <div class="card card-block bg_bg mt-3 " style="width: 300px;height:400px;">
                        <img id="<?php echo $col2;  ?>" class="color_clicked imageTwo"
                            src="data:image/jpeg;base64,{{ base64_encode($item->opposite_two_image) }}"
                            alt="{{ $item->opposite_two_image_text }}">
                        <h5 class="card-title mt-3 mb-3 category_text imageTwoText">{{ $item->opposite_two_image_text }}
                        </h5>
                    </div>
                </div>
...
btnSpeak.addEventListener('click', ()=> {
    var toSpeak = new SpeechSynthesisUtterance("{{ $item->opposite_two_image_text }}");
    var selectedVoiceName = "Microsoft Tolga - Turkish (Turkey)"
    voices.forEach((voice)=>{
        if(voice.name === selectedVoiceName){
            voice.name
