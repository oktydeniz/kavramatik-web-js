...
@section('content')
    ...
    <div class="container mt-2">
        <a href="{{route('eslestirme.miktarlar')}}" class="btn btn-sm btn-primary"> <i class="fa fa-plus"></i>
            Eşleştirme</a>
                ...
                <div class="col-md-4 col-sm-6 m-10">
                    <div class="card card-block bg_bg mt-3 " style="width: 300px;height:400px;">
                        <img id="<?php echo $col1;  ?>" class="color_clicked imageOne"
                            src="data:image/jpeg;base64,{{ base64_encode($item->quantitiy_one_image) }}"
                            alt="{{ $item->quantitiy_one_text }}">
                        <h5 class="card-title mt-3 mb-3 category_text imageOneText">{{ $item->quantitiy_one_text }}</h5>
                    </div>
                </div>
                <script>
...
btnSpeak.addEventListener('click', ()=> {
    var toSpeak = new SpeechSynthesisUtterance("{{ $item->quantitiy_one_text }}");
                ...
                <div class="col-md-4 col-sm-6 m-10">
                    <div class="card card-block bg_bg mt-3 " style="width: 300px;height:400px;">
                        <img id="<?php echo $col2;  ?>" class="color_clicked imageTwo"
                            src="data:image/jpeg;base64,{{ base64_encode($item->quantitiy_two_image) }}"
                            alt="{{ $item->quantitiy_two_text }}">
                        <h5 class="card-title mt-3 mb-3 category_text imageTwoText">{{ $item->quantitiy_two_text }}</h5>
                    </div>
                </div>
                <div class="col-md-2 col-sm-6"></div>
...
btnSpeak.addEventListener('click', ()=> {
    var toSpeak = new SpeechSynthesisUtterance("{{ $item->quantitiy_two_text }}");
    var selectedVoiceName = "Microsoft Tolga - Turkish (Turkey)"
    voices.forEach((voice)=>{
        if(voice.name === selectedVoiceName){
            voice.name
